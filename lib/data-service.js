  ...
  if (await isMySQLConnected()) {
    try {
      ...
      if (rows && rows.length > 0) {
        ...
        return {
          ...row,
          target_lat: row.target_lat !== null && row.target_lat !== undefined ? Number(row.target_lat) : -6.384288,
          target_lng: row.target_lng !== null && row.target_lng !== undefined ? Number(row.target_lng) : 106.869938,
          radius_meters: row.radius_meters ? Number(row.radius_meters) : 50,
          home_lat: row.home_lat !== null && row.home_lat !== undefined ? Number(row.home_lat) : -6.396742,
          home_lng: row.home_lng !== null && row.home_lng !== undefined ? Number(row.home_lng) : 106.839228,
...
export async function createStudentBySuperAdmin({
    ...
    try {
      ...
      const [studentRes] = await pool.query(
        ...
        [
          ...
          home_lat || -6.396742, home_lng || 106.839228, home_radius_meters || 50,
  ...
  const mockS = {
    ...
    home_lat: home_lat || -6.396742,
    home_lng: home_lng || 106.839228,
