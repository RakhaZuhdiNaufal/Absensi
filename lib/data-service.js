...
export async function updateUserPhoto(userId, photo) {
  if (await isMySQLConnected()) {
    try {
      ...
      try {
        await pool.query(
          `INSERT INTO profile_updates (user_id, action_type, photo) VALUES (?, 'upload_photo', ?)`,
          [userId, photo]
        );
      } catch (logErr) {
        console.warn('Warning inserting profile_updates photo log:', logErr.message);
      }
      return true;
    } catch (e) {
...
export async function updateStudentProfile(userId, { name, username, email, status, tempat_pkl, pembimbing_name, bio, jabatan, instansi, siswa_bimbingan, alamat_rumah, alamat_pkl, class: studentClass, nis, major, target_lat, target_lng, radius_meters, home_lat, home_lng, home_radius_meters }) {
  if (await isMySQLConnected()) {
    try {
      ...
      try {
        await pool.query(
          `INSERT INTO profile_updates (user_id, action_type, name, email, bio, alamat_rumah, alamat_pkl)
           VALUES (?, 'edit_profile', ?, ?, ?, ?, ?)`,
          [
            userId,
            name || null,
