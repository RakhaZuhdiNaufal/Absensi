...
async function isMySQLConnected() {
  ...
  try {
    ...
    if (dbConnectionStatus !== true) {
      ...
      try {
        await pool.query(`ALTER TABLE users MODIFY COLUMN photo LONGTEXT DEFAULT NULL`);
      } catch (_) {}
...
export async function findUserByCredential(credential) {
  ...
  if (await isMySQLConnected()) {
    try {
      let [rows] = await pool.query(
        `SELECT u.* FROM users u
         ...
         WHERE LOWER(u.username) = ? 
            ...
            OR LOWER(u.name) = ? 
            OR LOWER(s.nis) = ?
            OR REPLACE(LOWER(u.username), ' ', '') = ?
            OR REPLACE(LOWER(u.name), ' ', '') = ?
         LIMIT 1`,
        [cleanCred, cleanCred, cleanCred, cleanCred, noSpace, noSpace]
      );

      // Fallback kata kunci bawaan pembimbing / admin
      if ((!rows || rows.length === 0) && ['admin', 'pembimbing', 'pembimbing1', 'pak ridwan', 'pakridwan', 'ridwan', 'rdiwan'].includes(cleanCred)) {
      ...
      console.warn('MySQL Query Error findUserByCredential:', e.message);
    }
  }
  ...
  const user = mockUsers.find(u =>
    ...
    (u.name && u.name.toLowerCase() === cleanCred) ||
    (u.username && u.username.toLowerCase().replace(/\s+/g, '') === noSpace) ||
    (u.name && u.name.toLowerCase().replace(/\s+/g, '') === noSpace) ||
    (['admin', 'pembimbing', 'pak ridwan', 'pakridwan', 'ridwan'].includes(cleanCred) && u.role === 'admin')
  );
  ...
  const st = mockStudents.find(s => s.nis && s.nis.toLowerCase() === cleanCred);
  if (st) {
...
export async function updateUserPassword(userId, newPassword) {
  ...
  if (await isMySQLConnected()) {
    try {
      ...
      try {
        await pool.query(
          `INSERT INTO password_resets (user_id, action, status) VALUES (?, 'change_password', 'success')`,
          [userId]
        );
      } catch (logErr) {
        console.warn('Warning inserting password_resets log:', logErr.message);
      }
  ...
  if (await isMySQLConnected()) {
    try {
      ...
      try {
        ...
        console.warn('Warning inserting user_photos log:', logErr.message);
      }
      const user = mockUsers.find(u => u.id === Number(userId));
      if (user) {
        user.photo = photo;
      ...
      console.error('MySQL Error updateUserPhoto:', e.message);
      throw new Error('Gagal menyimpan foto profil: ' + e.message);
    }
  }
...
export const getAdminStats = getSuperAdminDashboard;
  ...
  if (await isMySQLConnected()) {
    try {
      if (name) {
        ...
        try {
          await pool.query(`UPDATE students SET pembimbing_name = ? WHERE pembimbing_id = ?`, [name, userId]);
        } catch (_) {}
      }
      if (username) {
      ...
      try {
        await pool.query(
          `INSERT INTO profile_updates (user_id, name, email, bio, alamat_rumah, alamat_pkl)
           VALUES (?, ?, ?, ?, ?, ?)`,
          [
            userId,
            name || null,
        ...
        console.warn('Warning inserting profile_updates log:', logErr.message);
      }
      return true;
    } catch (e) {
