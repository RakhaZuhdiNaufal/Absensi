...
let mockUsers = [
  {
    ...
    password: '$2a$10$eE0mE3n/.F9VjX0B7pYy6.wJdD9R74Z6Q0zH/j/u6Z7Q0zH/j/u6Z',
    role: 'super_admin',
  ...
  },
  {
    ...
    password: '$2a$10$eE0mE3n/.F9VjX0B7pYy6.wJdD9R74Z6Q0zH/j/u6Z',
    role: 'siswa',
    photo: '/default-avatar.png',
    created_at: new Date().toISOString()
  },
...
async function isMySQLConnected() {
  ...
  const now = Date.now();
  if (dbConnectionStatus !== null && (now - lastDbCheckTime < 30000)) {
...
export async function findUserById(id) {
    ...
    try {
      ...
      if (rows && rows.length > 0) {
        ...
        if (u.role === 'admin') {
          ...
          if (!u.username) u.username = 'Ridwan';
          if (!u.bio) {
  ...
  if (u) {
    ...
    if (userCopy.role === 'admin') {
      ...
      if (!userCopy.username) userCopy.username = 'Ridwan';
      if (!userCopy.bio) {
...
export async function findUserByCredential(credential) {
  ...
  const user = mockUsers.find(u =>
    ...
    (cleanCred === 'ridwan' && u.role === 'admin') ||
    (cleanCred === 'pembimbing1' && u.role === 'admin')
  );
  ...
  if (await isMySQLConnected()) {
    try {
      ...
      const [resUser] = await pool.query(
        ...
        [userName, cleanNipd, email, '$2a$10$eE0mE3n/.F9VjX0B7pYy6.wJdD9R74Z6Q0zH/j/u6Z', '/default-avatar.png']
      );
...
export async function updateUserPassword(userId, newPassword) {
  ...
  if (await isMySQLConnected()) {
    try {
      ...
      console.warn('MySQL Error updateUserPassword:', e.message);
    }
  }
...
export async function getTodayAttendance(studentId) {
  ...
  if (await isMySQLConnected()) {
    try {
      const [rows] = await pool.query(
        `SELECT * FROM attendance WHERE student_id = ? AND attendance_date = ? LIMIT 1`,
        [studentId, todayStr]
      );
  ...
  return mockAttendance.find(a => a.student_id === Number(studentId) && a.attendance_date === todayStr) || null;
}
...
export async function deleteTodayAttendance(studentId = null) {
  ...
  if (await isMySQLConnected()) {
    try {
      if (studentId) {
        await pool.query(
          `DELETE FROM attendance WHERE student_id = ? AND (attendance_date = ? OR attendance_date = CURDATE())`,
          [studentId, todayStr]
        );
      } else {
        await pool.query(
          `DELETE FROM attendance WHERE attendance_date = ? OR attendance_date = CURDATE()`,
          [todayStr]
        );
      }
    } catch (e) {
  ...
  if (studentId) {
    mockAttendance = mockAttendance.filter(a => !(a.student_id === Number(studentId) && a.attendance_date === todayStr));
  } else {
    ...
    try {
      const [result] = await pool.query(
        `INSERT INTO attendance (student_id, attendance_date, attendance_time, photo, latitude, longitude, location, reason, note, status, work_mode, device_id)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [student_id, attendance_date, attendance_time, photo, latitude, longitude, location, reason, note, cleanStatus, cleanWorkMode, device_id || null]
      );
      return { id: result.insertId, student_id, attendance_date, attendance_time, photo, latitude, longitude, location, reason, note, status: cleanStatus, work_mode: cleanWorkMode, device_id };
    } catch (e) {
...
export async function getAdminDashboard(pembimbingUserId) {
      ...
      if (studentIds.length > 0) {
        const [rowsToday] = await pool.query(
          `SELECT a.*, u.name as student_name, s.nis, s.class, s.tempat_pkl, u.photo as student_photo
           ...
           JOIN users u ON s.user_id = u.id
           WHERE a.attendance_date = ? AND a.student_id IN (?)
           ORDER BY a.created_at DESC`,
          [todayStr, studentIds]
        );
        attendanceToday = rowsToday || [];
  ...
  const todayStr = new Date().toISOString().split('T')[0];
  const attendanceToday = mockAttendance.filter(a => a.attendance_date === todayStr);
  const allAttendance = mockAttendance;
...
export async function updateStudentProfile(userId, { name, username, email, status, tempat_pkl, pembimbing_name, bio, jabatan, instansi, siswa_bimbingan, alamat_rumah, alamat_pkl, class: studentClass, nis, major }) {
  if (await isMySQLConnected()) {
    try {
      if (name) {
...
export async function createStudentBySuperAdmin({
    ...
    try {
      ...
      const [studentRes] = await pool.query(
        `INSERT INTO students (
          ...
          target_lat, target_lng, radius_meters, pembimbing_id, periode_mulai, periode_selesai
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          newUserId, cleanUsername, studentClass, major, tempat_pkl, alamat_rumah, alamat_pkl,
          target_lat || -6.384288, target_lng || 106.869938, radius_meters || 50,
  ...
  const mockS = {
    ...
    alamat_rumah,
    alamat_pkl,
    target_lat: target_lat || -6.384288,
    target_lng: target_lng || 106.869938,
